...
std::vector<uint8_t> MotorControl::getStatusBinary() const
{
    ...
    RaftDevice::genBinaryDataMsg(binBuf, RaftDeviceID::BUS_NUM_DIRECT_CONN, 
                                  0, getDeviceTypeIndex(), DeviceOnlineState::ONLINE, 0, data);
    return binBuf;
}
...
bool MotorControl::getDeviceTypeRecord(DeviceTypeRecordDynamic& devTypeRec) const
{
    ...
    //         3x4-byte int32s (steps), 1-byte flags, 16-byte pattern = 41 bytes after timestamp
    ...
    static const char* devInfoJson = R"~({"name":"MotorControl","desc":"Multi-axis Motor Controller","manu":"Robotical","type":"MotorControl")~"
        ...
        R"~({"n":"pattern","at":25,"t":"16s","f":"s","o":"str"})~"
        R"~(]}})~";
